  ...
  class Triangle {
    ...
    show() {
      ...
      if(round(this.color) === 0) {
        stroke(66, 133, 244);
      }
      if(round(this.color) === 1) {
        stroke(219, 68, 55);
      }
      if(round(this.color) === 2) {
        stroke(244, 180, 0);
      }
      if(round(this.color) === 3) {
        stroke(15, 157, 88);
      }
  
      rotate(this.angle*PI);
      
      if(round(this.shape)===0){
        triangle(this.x,this.y,this.x+this.side,this.y,this.x+this.side/2,this.y+sqrt(3)/2*this.side);
      } 
      if(round(this.shape)===1) {
        rect(this.x,this.y,this.side,this.side);
      }
      if(round(this.shape)===2) {
  ...
  function setup() {
    ...
    for(let i=0;i<numOfFig;i++) {
      t[i] = new Triangle(random(-50,width+50),random(-50,height+50),random(40,70),random(0,0.5),random(0.01,0.09),random(-0.5,2.5),random(-0.5, 3.5))
    }
  }
  ...
  function draw() {
    ...
    for(let i=0;i<numOfFig;i++) {
      if(i%2===0) {
        t[i].x = t[i].x + t[i].shift*cos(j*coeffT);
        t[i].y = t[i].y + t[i].shift*sin(j*coeffT);  
      } else {
        t[i].x = t[i].x + t[i].shift*sin(j*coeffT);
        t[i].y = t[i].y + t[i].shift*cos(j*coeffT);  
      }  
    }
